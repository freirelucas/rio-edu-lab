"""Download lote dos PDFs do Grupo Educação.

Mesma estrutura do 03_download_excels.py, mas para items do tipo 'PDF'.
Salva em data/raw/pdf/{id}.pdf. Resumível.

Uso:
  python3 analysis/07_download_pdfs.py
  python3 analysis/07_download_pdfs.py --limit 3   # smoke
"""
...
def main() -> int:
    ...
    items = [it for it in manifest["items"] if it.get("type") == "PDF"]
    if args.limit:
        items = items[: args.limit]

    print(f"target: PDF, total: {len(items)}, dest: {DEST.relative_to(ROOT)}")
    ...
    print(
        ...
        f"total={total_bytes:,} B ({total_bytes / 1024**2:.1f} MiB)"
    )
