...
- **Qualidade empírica** (v0.13): gold-set de ~50 pairs `(paper, predicted_category)` etiquetados à mão por categoria/banda de score → precision/recall + confusion matrix em [Qualidade do match](match-quality.md). Único jeito de saber se o matching é confiável (proxies indiretos não bastam).
